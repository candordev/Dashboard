...
const AllChatsScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    if (state.leaderGroups && state.leaderGroups[0] && state.leaderGroups[0].numbers) {
      // Assuming numbers have a structure that matches the NumberItem interface
      const sorted: NumberItem[] = [...state.leaderGroups[0].numbers].sort((a, b) => {
  ...
  return (
    <>
      ...
      <OuterView style={{ backgroundColor: colors.white, flexDirection: 'row', flex: 1, borderRadius: 20, overflow: 'visible', padding: 0}}>
        {/* Numbers Section */}
        <View style={{ flex: 1/4}}>
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                style={{
                    ...
                    backgroundColor: item.number === selectedPhoneNumber ? colors.lightergray : colors.white,
                    padding: 12,
                    paddingLeft: 15,
        ...
        <View style={{ width: 1, backgroundColor: colors.lightgray, opacity: 0.5 }}/>

        <View style={{ flex: 3/4, padding: 20 }}>
          {selectedPhoneNumber ? <ChatComponent phoneNumber={selectedPhoneNumber} /> : null}
        </View>
      </OuterView>
    </>
  );
