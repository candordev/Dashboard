...
const CSVImportComponent: React.FC<CSVImportComponentProps> = ({
    ...
    if (file) {
      ...
      Papa.parse<CSVRow>(file, {
        ...
        complete: async function (results) {
          ...
          const nonEmptyAssignees = Array.from(allAssignees).filter(
            (email) => email && email
          );

          const emailCheckResults = await checkEmailAccounts(
            Array.from(nonEmptyAssignees)
          );
          ...
          const usernameMap = new Map(
            emailCheckResults.map((result: { email: any; username: any }) => [
              result.email,
              result.username,
            ])
          );
          ...
          const newGroupedData = Array.from(groupedRows).map(
            ([key, assignees]) => {
  ...
  return (
    <>
      <Popover
        isVisible={
          isPopoverVisible || (assigneesToAdd.length > 0 && !allLeadersAdded)
        }
